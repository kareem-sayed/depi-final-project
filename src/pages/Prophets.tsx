...
const content = { 
  ar: { 
    ...
    subtitle: "سير وتاريخ 25 نبيّاً ورسولاً ذُكروا في القرآن الكريم، نأخذ من قصصهم العبرة والموعظة", 
  ...
  en: { 
    ...
    subtitle: "Biographies and history of 25 prophets mentioned in the Holy Quran, taking lessons from their stories.", 
    ...
    <div dir={lang === "ar" ? "rtl" : "ltr"} className="container mx-auto px-4 py-10 mt-20 font-cairo relative"> 
      ...
      <div className="bg-card text-card-foreground p-4 md:p-6 rounded-card shadow-soft border border-border mb-12 flex flex-col sm:flex-row gap-4 items-center justify-between"> 
        ...
        <div className="relative w-full sm:w-[420px]"> 
          ...
          /> 
          <svg className="w-5 h-5 text-forest absolute ltr:left-3.5 rtl:right-3.5 top-1/2 -translate-y-1/2 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24"> 
        ...
        <div className="flex items-center justify-between sm:justify-end w-full sm:w-auto gap-4"> 
          ...
          > 
            <span className={`w-2.5 h-2.5 rounded-full transition-colors ${onlyUlulAzm ? "bg-gold" : "bg-muted"}`}></span> 
            {t.ululAzmOnly} 
          </button> 
          ...
          <div className="px-4 py-2 bg-forest/10 dark:bg-forest-dark text-forest-dark dark:text-gold-light rounded-xl border border-forest/20 text-xs font-bold flex items-center gap-1.5"> 
            <span>{t.count}</span> 
            <span className="text-sm font-extrabold">{filteredProphets.length}</span> 
          </div> 
        </div> 
      </div> 
      ...
      {filteredProphets.length > 0 ? ( 
            ...
            > 
              ...
              <div className="pt-4 border-t border-border flex items-center justify-end text-sm font-extrabold text-forest-dark dark:text-gold group-hover:text-gold-dark transition-colors"> 
                <span className="flex items-center gap-1.5 ltr:group-hover:translate-x-1 rtl:group-hover:-translate-x-1 transition-transform"> 
                  {t.readStory} 
                  <span className="text-lg leading-none rtl:rotate-0 ltr:rotate-180">&larr;</span> 
                </span> 
              </div> 
            </Link> 
          ))} 
        ...
        <div className="text-center py-20 bg-card rounded-card border-2 border-dashed border-border shadow-soft max-w-lg mx-auto"> 
          <p className="text-forest-dark dark:text-gold text-lg font-bold">{t.noResults} "{searchQuery}"</p> 
          <button 
            onClick={() => { setSearchQuery(""); setOnlyUlulAzm(false); }} 
