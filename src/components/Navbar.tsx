    ...
    <>
      ...
      >
        <nav className="mx-auto flex max-w-7xl items-center justify-between px-6 lg:px-8">
          <div className={`flex md:hidden ${currentLang === "en" ? "order-1" : "order-3"}`}>
            <button
              ...
              <div className="relative">
                ...
                {dropdownOpen && (
                  <>
                    ...
                    <div className={`absolute mt-2 w-40 rounded-xl bg-background p-2 shadow-xl ring-1 ring-black/5 z-20 ${currentLang === "ar" ? "left-0 origin-top-left" : "right-0 origin-top-right"}`}>
                      <Link
                        to="/dashboard"
      ...
      <div
        ...
        className={`fixed inset-y-0 z-[200] w-full max-w-xs bg-background p-6 shadow-2xl transition-transform duration-300 ease-in-out md:hidden ${
          ...
          isOpen ? "translate-x-0" : currentLang === "ar" ? "translate-x-full" : "-translate-x-full"
        }`}
      >
        ...
        {isLoggedIn && (
          <div className="mb-6 p-3 rounded-xl bg-muted/50">
            ...
            <span className="block text-xs text-foreground/60 capitalize">{user?.role || "..."}</span>
          </div>
        )}

        <div className={`space-y-4 border-b border-border pb-6 ${currentLang === "ar" ? "text-right" : "text-left"}`}>
          {navItems.map((item) => (
            <Link
              key={item.to}
              to={item.to}
